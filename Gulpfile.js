...
function watch_eslint(done) {
    gulp.watch(ts_sources, { ignoreInitial: false }, lint);
    done();
}
    ...
    devserver.get("*", (req, res) => {
        ...
        let index = _index.replace(/[{][{]\s*(\w+)\s*[}][}]/g, (_, parameter) => {
            switch (parameter) {
                ...
                case "KIDSGO_LIVE_RELOAD":
                    return `<script async src="//${req.hostname}:35707/livereload.js"></script>`;
                case "OG_KIDSGO_TITLE":
                    return "";
                case "OG_KIDSGO_URL":
                    return "";
                case "OG_KIDSGO_IMAGE":
                    return "";
                case "OG_KIDSGO_DESCRIPTION":
                    return "";
                case "KIDSGO_VERSION_HASH_DOTJS":
                    return "js";
                case "KIDSGO_VERSION_DOTCSS":
                    return "css";
                case "KIDSGO_LANGUAGE_VERSION_DOTJS":
                    return "js";
            }
            return "{{" + parameter + "}}";
        });
...
function minify_index(done) {
    ...
    let index = _index.replace(/[{][{]\s*(\w+)\s*[}][}]/g, (_, parameter) => {
        switch (parameter) {
            case "KIDSGO_VENDOR_HASH_DOTJS":
                return process.env["KIDSGO_VENDOR_HASH"] + ".js";
            case "KIDSGO_VERSION_HASH_DOTJS":
                return process.env["KIDSGO_VERSION_HASH"] + ".js";
        }
        return "{{" + parameter + "}}";
    });
